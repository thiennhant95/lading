...
@section('content')
    ...
    <header>
        <div class="flexrowbetween hdtop main">
            <div class="logo"><a href="{{ url('user') }}"><img src="{{url('images/common_img/logoLPuser.png')}}" alt="スマオク！ 競争見積り オークション 業界初!？ 中古車買取のビッグバンサイト！"></a></div>
                ...
                <div class="tbbox">
                    ...
                    <tr>
                        ...
                        @foreach($list_makers as $makers)
                            ...
                            @if(session()->has('makers') && session()->get('makers') == $makers->id)
                                <option value="{{ $makers->id }}" selected="selected">{{ $makers->name }}</option>
                            @endif
                        @endforeach
                        </select></td>
                    </tr>
                    <tr>
                        ...
                        @foreach($list_cars as $cars)
                            ...
                            @if(session()->has('cars') && session()->get('cars') == $cars->id)
                                <option value="{{ $cars->id }}" selected="selected">{{ $cars->name }}</option>
                            @endif
                        @endforeach
                        </select></td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            @if(session()->has('year'))
                            <?php 
                                $explode = explode(',',session()->get('year')); 
                                $year_type = $explode[0];
                                $car_year = $explode[1];
                                $car_month = $explode[2];
                            ?>
                            <select class="w45" id="infor_car_year_type" name="infor_car_year_type">
                                <option value="H" @if($year_type == "H") selected="selected" @endif>H</option>
                                <option value="S" @if($year_type == "S") selected="selected" @endif>S</option>
                            </select>
                            <input name="infor_car_year" id="infor_car_year" class="w15" type="text" value="{{ $car_year }}"/> 年
                            <input name="infor_car_month" class="w15" id="infor_car_month" type="text" value="{{ $car_month }}"/> 月
                            @else
                            <select class="w45" id="infor_car_year_type" name="infor_car_year_type">
                                <option value="H">H</option>
                                <option value="S">S</option>
                            </select>
                            <input name="infor_car_year" id="infor_car_year" class="w15" type="text" value=""/> 年
                            <input name="infor_car_month" class="w15" id="infor_car_month" type="text" value=""/> 月
                            @endif
                        </td>
                    </tr>
                    <tr>
                        <th><span class="ninni">任意</span> 走行距離</th>
                        <td><input type="text" value="@if(session()->has('mileage')){{ session()->get('mileage') }}@endif" name="seller_mileage" id="seller_mileage" size="" tabindex="" accesskey="" placeholder=""></td>
                    </tr>
                </table>
                <table class="tbstyle">
                    <tr>
                        <th><span class="hinsu">必須</span> TEL</th>
                        <td><input type="text" value="@if(session()->has('phone')){{ session()->get('phone') }}@endif" name="seller_tel" id="seller_tel" size="" tabindex="" accesskey="" placeholder="例）09012345678" ></td>
                    </tr>
                    <tr>
                        <th><span class="hinsu">必須</span> 名前</th>
                        <td><input type="text" value="@if(session()->has('name')){{ session()->get('name') }}@endif" name="seller_name" id="seller_name" size="" tabindex="" accesskey="" placeholder="例）山田　太郎"></td>
                    </tr>
                    <tr>
                        ...
                        @foreach($list_zones as $zones)
                            ...
                            @if(session()->has('zone') && session()->get('zone') == $zones->id)
                                <option value="{{ $zones->id }}" selected="selected">{{ $zones->name }}</option>
                            @endif
                        @endforeach
                        </select></td>
                    </tr>
                    <tr>
                        ...
                        <td><input type="text" value="@if(session()->has('email')){{ session()->get('email') }}@endif" name="seller_email" id="seller_email" size="" tabindex="" accesskey="" placeholder="例）yamada@smart.com"></td>
                    </tr>
                </table>
                </div>
        ...
        <div class="boxteltop">
            ...
            <div class="tbbox">
                <table class="tbstyle">
                    ...
                    <tr>
                        <th><span class="ninni">任意</span> 年式</th>
                        <td><select class="w45" id="seller_date_year1" name="infor_car_year_type">
                                <option value="H" selected="selected">H</option>
                                <option value="S">S</option>
                            </select>
                            <input name="infor_car_year" id="infor_car_year1" class="w15" type="text" value="" style="width: 10% !important"/> 年
                            <input name="infor_car_month" class="w15" id="infor_car_month1" type="text" value="" style="width: 10% !important"/> 月</td>
                    </tr>
                    <tr>
                        <th><span class="ninni">任意</span> 走行距離</th>
            ...
            <div class="tbbox">
                <table class="tbstyle">
                    ...
                    <tr>
                        <th><span class="ninni">任意</span> 年式</th>
                        <td>
                            <select class="w45" id="infor_car_year_type2" name="infor_car_year_type">
                                <option value="H" selected="selected">H</option>
                                <option value="S">S</option>
                            </select>
                            <input name="infor_car_year" id="infor_car_year2" class="w15" type="text" value="" style="width: 15% !important"/> 年
                            <input name="infor_car_month" class="w15" id="infor_car_month2" type="text" value="" style="width: 15% !important"/> 月</td>
                    </tr>
                    <tr>
                        <th><span class="ninni">任意</span> 走行距離</th>
